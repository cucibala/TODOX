...
- **项目管理**：多项目分类管理
- **任务管理**：新增/编辑/完成/置顶，优先级与截止日期
- **子任务**：权重计算整体进度，支持必填输入
- **进度追踪**：文字 + 图片记录，支持粘贴图片（Ctrl+V）
- **图片附件**：多图上传、缩略图预览、点击放大
- **内置 API 助手**：创建/管理任务、制定计划、智能拆解
...
内置 API 助手（DeepSeek）：帮你创建任务、管理任务、制定计划，并基于项目上下文给出建议。

### 功能
- 角色系统：通用助手、项目助手（可扩展）
- 项目上下文：可关联项目，自动读取项目/任务/子任务信息
- 任务操作：用自然语言创建/更新任务、添加子任务、调整计划
- 流式回复：长文本分段输出，阅读更流畅
- 进度提示：批量操作显示实时状态与耗时
- 对话管理：新建/切换/删除/清空，对话本地持久化
- 密钥安全：DeepSeek API 密钥本地加密保存，可随时更新/删除

### 使用方法
- 在“设置”中配置 DeepSeek API 密钥
- 打开“聊天”页面，选择 AI 角色（项目助手可勾选关联项目）
- 直接用自然语言提出需求，例如：
  - 为项目 A 创建“发布 v3.5.0”计划并拆解任务
  - 把登录模块拆成具体子任务并标注重要程度
  - 给任务 X 新增子任务：补齐单元测试
- 使用 Ctrl+Enter 发送消息
