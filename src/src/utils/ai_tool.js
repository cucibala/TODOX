...
6. 只返回 JSON 数组，不要其他内容`
  ...
  new AIFunction(
    createTool(
      'deleteSubtask',
      '删除指定任务的指定子任务。用于移除不需要的子任务，如"删除第一个任务的第2个子任务"、"把环境准备的第3个子任务删掉"等',
      {
        taskId: {
          type: 'number',
          description: '任务ID'
  ...
  new AIFunction(
    createTool(
      'deleteTask',
      '删除指定的任务。用于移除不需要的任务，如"删除第3个任务"、"删除环境准备这个任务"等',
      {
        taskId: {
          type: 'number',
...
export class AITool {
  ...
  async execute(functionName, args = {}, onProgress = null) {
    console.log(allFunctions, 'allFunctions', functionName, args, onProgress)
